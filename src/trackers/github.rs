use super::tracker::{Issue, IssueTracker};
...
impl IssueTracker for Github {
    fn build_request_url(&self) -> String {
        format!(
            "{}/repos/{}/{}/issues",
            self.base_tracker_url, self.owner, self.repo
        )
    }
    ...
    async fn fetch_issues(&self) -> Vec<Issue> {
        ...
        let query_string = build_query_string(vec![("per_page", &self.issues_per_request)]);
        ...
        match response.status() {
            ...
            status_code => {
                panic!(
                    "Received error reaching out to github API: {:?}",
                    status_code
                );
            }
        }
        ...
        let issues: Vec<Issue> = match response.json().await {
            Ok(issues) => issues,
            Err(err) => panic!(
                "Problem marshaling response data into issue type, {:?}",
                err
            ),
        };
...
mod tests {
    ...
    pub async fn build_mock_server(
        method_type: &str,
        status: StatusCode,
        ...
        let response = match json_body {
            Some(data) => ResponseTemplate::new(status).set_body_json(data),
    ...
    mod fetch_issues {
        ...
        async fn should_handle_valid_response() {
            ...
            assert_eq!(
                issues[0].html_url,
                "https://github.com/Kjoedicker/snitch-lab/issues/650"
            );
        ...
        #[tokio::test]
        #[should_panic(
            expected = "Problem marshaling response data into issue type, reqwest::Error { kind: Decode"
        )]
